...
from gsima.environments.vizdoom_basic.schema import SUPPORTED_ACTIONS, VizdoomAction


class VizdoomBasicAdapter(BaseAdapter):
    """Adapter for ViZDoom Basic environments.
    ...
    def __init__(self, env):
        ...
        self._combat_search_pattern = [
            VizdoomAction.MOVE_LEFT.name,
            VizdoomAction.MOVE_RIGHT.name,
            VizdoomAction.SHOOT.name,
        ]
        logging.debug("VizdoomBasicAdapter initialized with action map: %s", self.action_map)

    def _get_env_action(self, action: VizdoomAction):
        ...
        if game is not None and button_map is not None:
            desired_button = {
                VizdoomAction.MOVE_LEFT: vizdoom.Button.MOVE_LEFT,
                VizdoomAction.MOVE_RIGHT: vizdoom.Button.MOVE_RIGHT,
                VizdoomAction.SHOOT: vizdoom.Button.ATTACK,
            }.get(action)
        ...
        if action == VizdoomAction.SHOOT:
            return 1
        if action == VizdoomAction.MOVE_RIGHT:
            return 2
        if action == VizdoomAction.MOVE_LEFT:
            return 3
        if action == VizdoomAction.STOP:
            return -1
    ...
    def translate_action(self, action_name: str) -> Any:
        ...
        if action is None:
            ...
            return self.action_map[VizdoomAction.SHOOT.name]
        return action
